...
-- Total TABLES : 52 (toutes en français)
-- Total INDEXES : 25 (ajout de 3 index pour les abonnements)
--
-- Organisation des tables par rôle :
-- - Profils Utilisateurs (role: user) : profils_utilisateurs, preferences_utilisateurs, 
--   historique_recherches_utilisateurs, statistiques_evenements_utilisateurs
-- - Profils Administrateurs (role: admin) : profils_admin
-- - Profils Organisateurs (role: organizer) : profils_organisateurs
--
...
--
-- Ajout de la table profils_admin :
-- 1. Table similaire à profils_organisateurs mais sans : url_site_web, biographie,
--    type_organisation, numero_immatriculation, taille_entreprise, statut_verification,
--    onboarding_termine_le
--
-- Optimisation des duplications (2025-01-XX) :
-- 1. Suppression des colonnes dupliquées dans profils_utilisateurs :
--    - telephone, code_pays, code_langue, fuseau_horaire, url_avatar
--    Ces données sont maintenant uniquement dans utilisateurs pour éviter la duplication
-- 2. profils_utilisateurs contient maintenant uniquement :
--    - opt_in_marketing : consentement RGPD pour communications marketing
--    - categories_preferees : catégories préférées pour recommandations personnalisées
-- 3. Le thème/mode sombre est géré côté client (localStorage) et n'est pas stocké en BDD
--    pour améliorer les performances et réduire les requêtes inutiles
-- 4. Les champs communs entre profils_admin et profils_organisateurs sont intentionnels
--    car ils représentent des informations professionnelles similaires mais dans des contextes différents
--
