...
-- ============================================================
-- Statistiques de vues d'événements
-- ============================================================
CREATE TABLE IF NOT EXISTS vues_evenements (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    type_vue VARCHAR(20) NOT NULL DEFAULT 'page'
        CHECK (type_vue IN ('page', 'listing', 'search', 'share', 'embed')),
    duree_vue_secondes INTEGER,
    cree_le TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE IF NOT EXISTS invitations_evenements (
    ...
    email_destinataire public.citext,
...
CREATE TABLE IF NOT EXISTS types_accessibilite (
    ...
    code VARCHAR(50) NOT NULL UNIQUE
        CHECK (code IN ('general', 'hearing', 'visual', 'parking', 'toilet', 'pets')),
    libelle VARCHAR(255) NOT NULL,
...
-- Index supprimé : la table modes_paiement_evenements n'existe pas
...
CREATE INDEX IF NOT EXISTS idx_vues_evenements_evenement ON vues_evenements(id_evenement, cree_le);
CREATE INDEX IF NOT EXISTS idx_vues_evenements_utilisateur ON vues_evenements(id_utilisateur, cree_le);
CREATE INDEX IF NOT EXISTS idx_vues_evenements_date ON vues_evenements(cree_le);

-- ------------------------------------------------------------
-- Modes de paiement (table de référence)
-- ------------------------------------------------------------
CREATE TABLE IF NOT EXISTS modes_paiement (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    code VARCHAR(50) NOT NULL UNIQUE
        CHECK (code IN ('espace', 'mvola', 'orange', 'airtel', 'carte_bancaire', 'virement', 'cheque', 'autre')),
    libelle VARCHAR(255) NOT NULL,
    description TEXT,
    icone VARCHAR(120),
    est_actif BOOLEAN NOT NULL DEFAULT TRUE,
...
CREATE TABLE IF NOT EXISTS factures_billets (
    ...
    id_mode_paiement BIGINT REFERENCES modes_paiement(id) ON DELETE SET NULL,
    devise currency_code NOT NULL DEFAULT 'MGA',
...
CREATE TABLE IF NOT EXISTS historique_paiements_billets (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_facture BIGINT NOT NULL REFERENCES factures_billets(id),
...
CREATE INDEX IF NOT EXISTS idx_modes_paiement_code ON modes_paiement(code);
CREATE INDEX IF NOT EXISTS idx_modes_paiement_actif ON modes_paiement(est_actif) WHERE est_actif = TRUE;
...
-- ============================================================
-- Transactions de paiement mobile (pour abonnements) - Version simplifiée
-- ============================================================

-- Table principale des transactions de paiement mobile
-- Version sans les champs de sécurité optionnels
CREATE TABLE IF NOT EXISTS transactions_paiement_mobile (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    ...
    -- Informations sur le type de paiement
    operateur_mobile TEXT NOT NULL
        CHECK (operateur_mobile IN ('mvola', 'orange', 'airtel', 'espace')),
    type_paiement TEXT NOT NULL DEFAULT 'abonnement'
        CHECK (type_paiement IN ('abonnement', 'renouvellement', 'mise_a_niveau')),
    
    -- Informations de transaction
    numero_telephone phone_e164 NOT NULL,
    numero_transaction_operateur TEXT,
    montant NUMERIC(12,2) NOT NULL CHECK (montant > 0),
    devise currency_code NOT NULL DEFAULT 'MGA',
    
    -- Statuts
    statut_paiement payment_status_enum NOT NULL DEFAULT 'initiated',
    
    -- Réconciliation
    date_reconciliation TIMESTAMPTZ,
    reference_reconciliation TEXT,
    ...
    CHECK (confirme_le IS NULL OR confirme_le > initie_le),
    CHECK (annule_le IS NULL OR annule_le > initie_le),
    CHECK (expire_le > initie_le)
);

-- Table d'historique des statuts de transaction
CREATE TABLE IF NOT EXISTS historique_statuts_transactions (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_transaction BIGINT NOT NULL REFERENCES transactions_paiement_mobile(id),
    statut_de payment_status_enum,
    statut_vers payment_status_enum NOT NULL,
    raison TEXT,
    code_erreur TEXT,
...
-- Table de callback/notification des opérateurs
CREATE TABLE IF NOT EXISTS notifications_operateurs (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    id_transaction BIGINT REFERENCES transactions_paiement_mobile(id),
    operateur TEXT NOT NULL,
    type_notification TEXT NOT NULL
        CHECK (type_notification IN ('confirmation', 'annulation', 'erreur')),
    
    -- Données reçues
    donnees_recues JSONB NOT NULL,
    signature_recue TEXT,
    ip_source INET,
    
    -- Traitement
    traitee BOOLEAN NOT NULL DEFAULT FALSE,
    resultat_traitement TEXT,
    reponse_envoyee JSONB,
...
-- ============================================================
-- Index pour les transactions de paiement mobile
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_transactions_reference ON transactions_paiement_mobile(reference_transaction);
CREATE INDEX IF NOT EXISTS idx_transactions_facture ON transactions_paiement_mobile(id_facture);
CREATE INDEX IF NOT EXISTS idx_transactions_utilisateur ON transactions_paiement_mobile(id_utilisateur);
CREATE INDEX IF NOT EXISTS idx_transactions_organisateur ON transactions_paiement_mobile(id_profil_organisateur);
CREATE INDEX IF NOT EXISTS idx_transactions_statut ON transactions_paiement_mobile(statut_paiement);
CREATE INDEX IF NOT EXISTS idx_transactions_date_creation ON transactions_paiement_mobile(initie_le);
CREATE INDEX IF NOT EXISTS idx_transactions_operateur_numero ON transactions_paiement_mobile(operateur_mobile, numero_telephone);
CREATE INDEX IF NOT EXISTS idx_transactions_numero_operateur ON transactions_paiement_mobile(numero_transaction_operateur) WHERE numero_transaction_operateur IS NOT NULL;

CREATE INDEX IF NOT EXISTS idx_historique_transactions ON historique_statuts_transactions(id_transaction, cree_le);
CREATE INDEX IF NOT EXISTS idx_notifications_transactions ON notifications_operateurs(id_transaction, traitee, cree_le);
...
CREATE OR REPLACE FUNCTION generer_reference_transaction()
RETURNS TEXT AS $$
DECLARE
    nouvelle_reference TEXT;
BEGIN
    nouvelle_reference := 'SUB-' || TO_CHAR(NOW(), 'YYYYMMDD') || '-' || LPAD(NEXTVAL('sequence_numero_facture')::TEXT, 6, '0');
    RETURN nouvelle_reference;
END;
$$ LANGUAGE plpgsql;

-- Trigger pour générer automatiquement la référence de transaction
CREATE OR REPLACE FUNCTION trigger_generer_reference_transaction()
RETURNS TRIGGER AS $$
BEGIN
    IF NEW.reference_transaction IS NULL THEN
        NEW.reference_transaction := generer_reference_transaction();
    END IF;
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trg_generer_reference_transaction
    BEFORE INSERT ON transactions_paiement_mobile
    FOR EACH ROW
    EXECUTE FUNCTION trigger_generer_reference_transaction();

-- Fonction pour marquer une transaction comme expirée
CREATE OR REPLACE FUNCTION marquer_transactions_expirees()
RETURNS INTEGER AS $$
DECLARE
    nb_expirees INTEGER;
BEGIN
    UPDATE transactions_paiement_mobile
    SET statut_paiement = 'failed',
        modifie_le = NOW()
    WHERE statut_paiement IN ('initiated', 'processing')
      AND expire_le < NOW()
      AND confirme_le IS NULL
      AND annule_le IS NULL;
    
    GET DIAGNOSTICS nb_expirees = ROW_COUNT;
    RETURN nb_expirees;
END;
$$ LANGUAGE plpgsql;

-- Fonction pour obtenir le statut actuel d'une transaction
CREATE OR REPLACE FUNCTION obtenir_statut_transaction(p_reference_transaction TEXT)
RETURNS TABLE (
    reference_transaction TEXT,
    statut_paiement payment_status_enum,
    montant NUMERIC,
    operateur_mobile TEXT,
    numero_telephone TEXT,
    initie_le TIMESTAMPTZ,
    confirme_le TIMESTAMPTZ,
    facture_associee TEXT
) AS $$
BEGIN
    RETURN QUERY
    SELECT 
        tp.reference_transaction,
        tp.statut_paiement,
        tp.montant,
        tp.operateur_mobile,
        tp.numero_telephone,
    ...
    JOIN factures_abonnements fa ON tp.id_facture = fa.id
    WHERE tp.reference_transaction = p_reference_transaction;
END;
$$ LANGUAGE plpgsql;

-- ============================================================
-- Vues utiles pour les transactions
-- ============================================================

-- Vue des transactions en attente
CREATE OR REPLACE VIEW vue_transactions_en_attente AS
SELECT 
    tp.id,
    tp.reference_transaction,
    tp.id_facture,
    ...
    tp.id_profil_organisateur,
    po.nom_affichage AS organisateur,
    tp.operateur_mobile,
    tp.numero_telephone,
    tp.montant,
    tp.statut_paiement,
    ...
    tp.expire_le,
    CASE 
        WHEN tp.expire_le < NOW() THEN 'expiree'
        ELSE 'valide'
    END AS statut_expiration
FROM transactions_paiement_mobile tp
JOIN utilisateurs u ON tp.id_utilisateur = u.id
JOIN profils_organisateurs po ON tp.id_profil_organisateur = po.id
JOIN factures_abonnements fa ON tp.id_facture = fa.id
WHERE tp.statut_paiement IN ('initiated', 'processing')
  AND tp.annule_le IS NULL
  AND tp.confirme_le IS NULL;

-- Vue des transactions réussies par mois
CREATE OR REPLACE VIEW vue_transactions_reussies_par_mois AS
SELECT 
    DATE_TRUNC('month', tp.confirme_le) AS mois,
    tp.operateur_mobile,
    COUNT(*) AS nombre_transactions,
    SUM(tp.montant) AS montant_total,
    AVG(tp.montant) AS montant_moyen
FROM transactions_paiement_mobile tp
WHERE tp.statut_paiement = 'paid'
  AND tp.confirme_le IS NOT NULL
GROUP BY DATE_TRUNC('month', tp.confirme_le), tp.operateur_mobile;

-- Vue des transactions par organisateur
CREATE OR REPLACE VIEW vue_transactions_par_organisateur AS
SELECT 
    po.id AS id_organisateur,
    po.nom_affichage AS organisateur,
    COUNT(tp.id) AS total_transactions,
    COUNT(CASE WHEN tp.statut_paiement = 'paid' THEN 1 END) AS transactions_reussies,
    COUNT(CASE WHEN tp.statut_paiement IN ('initiated', 'processing') THEN 1 END) AS transactions_en_cours,
    COUNT(CASE WHEN tp.statut_paiement = 'failed' THEN 1 END) AS transactions_echouees,
    SUM(CASE WHEN tp.statut_paiement = 'paid' THEN tp.montant ELSE 0 END) AS montant_total_reussi
FROM profils_organisateurs po
LEFT JOIN transactions_paiement_mobile tp ON po.id = tp.id_profil_organisateur
GROUP BY po.id, po.nom_affichage;

-- ============================================================
-- Grant permissions
-- ============================================================
GRANT SELECT, INSERT, UPDATE, DELETE ON transactions_paiement_mobile TO aiolia_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON historique_statuts_transactions TO aiolia_user;
GRANT SELECT, INSERT, UPDATE, DELETE ON notifications_operateurs TO aiolia_user;

GRANT SELECT ON vue_transactions_en_attente TO aiolia_user;
GRANT SELECT ON vue_transactions_reussies_par_mois TO aiolia_user;
GRANT SELECT ON vue_transactions_par_organisateur TO aiolia_user;

GRANT EXECUTE ON FUNCTION generer_reference_transaction() TO aiolia_user;
GRANT EXECUTE ON FUNCTION marquer_transactions_expirees() TO aiolia_user;
GRANT EXECUTE ON FUNCTION obtenir_statut_transaction(TEXT) TO aiolia_user;
