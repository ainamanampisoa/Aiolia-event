...
CREATE TABLE IF NOT EXISTS venues (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    organizer_id BIGINT REFERENCES organizer_profiles(id) ON DELETE SET NULL,
    name TEXT NOT NULL,
    ...
    region TEXT,
    postal_code TEXT,
    country_code CHAR(2) NOT NULL DEFAULT 'MG',
...
CREATE UNIQUE INDEX IF NOT EXISTS uq_venue_spaces_default
    ON venue_spaces(venue_id)
    WHERE is_default;
...
CREATE TABLE IF NOT EXISTS events (
    ...
    main_space_id BIGINT REFERENCES venue_spaces(id) ON DELETE SET NULL,
    ...
    event_format VARCHAR(20) NOT NULL DEFAULT 'in_person'
    ...
    CHECK (sales_starts_at IS NULL OR sales_ends_at IS NULL OR sales_starts_at <= sales_ends_at)
);
...
CREATE TABLE IF NOT EXISTS event_sessions (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    event_id BIGINT NOT NULL REFERENCES events(id),
    venue_space_id BIGINT REFERENCES venue_spaces(id) ON DELETE SET NULL,
    title TEXT,
    description TEXT,
    starts_at TIMESTAMPTZ NOT NULL,
    ends_at TIMESTAMPTZ NOT NULL,
...
CREATE TABLE IF NOT EXISTS event_accessibility_links (
    event_id BIGINT NOT NULL REFERENCES aiolia.events(id) ON DELETE CASCADE,
    type_accessibilite_id BIGINT NOT NULL REFERENCES aiolia.types_accessibilite(id) ON DELETE CASCADE,
    PRIMARY KEY (event_id, type_accessibilite_id)
);
...
CREATE TABLE IF NOT EXISTS cart_items (
    ...
    event_id BIGINT REFERENCES aiolia.events(id),
    ticket_type_id BIGINT NOT NULL REFERENCES ticket_types(id),
    adult_ticket_type_id BIGINT REFERENCES aiolia.ticket_types(id),
    child_ticket_type_id BIGINT REFERENCES aiolia.ticket_types(id),
    quantity INTEGER NOT NULL CHECK (quantity > 0),
    adult_quantity INTEGER DEFAULT 0 CHECK (adult_quantity >= 0),
    child_quantity INTEGER DEFAULT 0 CHECK (child_quantity >= 0),
    unit_price NUMERIC(12,2) NOT NULL,
    adult_price NUMERIC(12,2) DEFAULT 0,
    child_price NUMERIC(12,2) DEFAULT 0,
    total_price NUMERIC(12,2) NOT NULL,
    cart_key TEXT,
    created_at TIMESTAMPTZ NOT NULL DEFAULT now()
    -- Note: La contrainte UNIQUE (cart_id, ticket_type_id) a été supprimée
    -- pour permettre plusieurs items pour le même événement avec différents types (adulte/enfant)
);
...
-- Table des transactions de paiement (MVola / autres providers)
CREATE TABLE IF NOT EXISTS payment_transactions (
    id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    order_id BIGINT NOT NULL REFERENCES orders(id) ON DELETE CASCADE,
    mvola_correlation_id TEXT UNIQUE,
    mvola_transaction_id TEXT,
...
CREATE INDEX IF NOT EXISTS idx_payment_transactions_correlation_id
    ON payment_transactions(mvola_correlation_id);
...
CREATE INDEX IF NOT EXISTS idx_events_organizer ON events(organizer_id, starts_at);
CREATE INDEX IF NOT EXISTS idx_events_venue ON events(venue_id, main_space_id);
...
-- Index pour les tables de panier
...
GRANT USAGE, CREATE ON SCHEMA aiolia TO aiolia_user;
